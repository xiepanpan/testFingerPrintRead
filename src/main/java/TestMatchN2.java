 ...
 * @Description:
 */
public class TestMatchN2 {
    private static Logger logger = LoggerFactory.getLogger(TestMatchN2.class);
    ...
    public static void main(String[] args) {
        ...
        byte[] pFeatureLib = new byte[10*256];
...
//        byte[] pFeaTmp3 = toBytes(str);
        ...
        NativeLong processReturnCode = D5ScannerS772.instanceDll.D5Process(pImage, pFeature);
        System.out.println("指纹转成特征值返回值："+processReturnCode);
        if (processReturnCode.longValue()==0) {
            System.out.println("提取成功");
        }else if (processReturnCode.longValue() == -1) {
        ...
        //6.2 字符串转byte数组
        byte[] datas = fileToByteArray("E:\\1.fea");//图片转为字节数组
        System.arraycopy(datas,0,pFeatureLib,0,datas.length);
        byte[] datas2 = fileToByteArray("E:\\我的食指.fea");//图片转为字节数组
...
//        ByteBuffer pFeature2 = ByteBuffer.wrap(pFeatureLib);
        int lFingernum = 10;
        try {
            NativeLong matchReturnCode = D5MatchN(pFeature, pFeatureLib,lFingernum,uRotate, uLevel);
            System.out.println("匹配返回值："+matchReturnCode);
            if (matchReturnCode.longValue() >= 0&&matchReturnCode.longValue()<10) {
                System.out.println("OK");
                for (i = 0; i < 3; i++)
                {
                    D5ScannerS772.instanceDll.D5Beep(uDeviceID,uMS);
                    Thread.sleep(40);
                }

            }else {
                D5ScannerS772.instanceDll.D5Beep(uDeviceID, (short) 160);
            }
...
//            }
    ...
    private static NativeLong D5MatchN(ByteBuffer pFeature, byte[] pFeature2, int i, short uRotate, short uLevel) {
        ByteBuffer pFeature3 = ByteBuffer.wrap(pFeature2);
        NativeLong l = D5ScannerS772.instanceDll.D5MatchN(pFeature, pFeature3,i, uRotate, uLevel);
        return l;
    }

//    private static long D5MatchN2(ByteBuffer pFeature, byte[] pFeature2, int i, short uRotate, short uLevel) {
//        JNative n = null;
//        try
//        {
//            n = new JNative("D5ScannerS77.dll", "D5MatchN");
